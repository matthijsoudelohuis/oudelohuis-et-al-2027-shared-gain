...
plt.tight_layout()
sns.despine(top=True,right=True,offset=3)
my_savefig(fig,figdir,'Poprate_Excerpt_LayerLabeled_%s_%s' % ('_'.join(protocols),sessions[ises].session_id),formats=['png'])
...
for ises in range(nSessions):
    respmat                = zscore(sessions[ises].calciumdata.T.to_numpy(), axis=1)
    # respmat                = sessions[ises].calciumdata.T.to_numpy()
    # respmat                = respmat / np.nanmax(respmat, axis=1, keepdims=True)
    # respmat                = block_reduce(respmat, block_size=(1,2), func=np.mean, cval=np.mean(respmat))
    ...
    for ialp,alp in enumerate(arealabelpairs):
        ...
        # corrdata[ialp,ises]      = np.corrcoef(np.nanmean(respmat[idx_N1,:],axis=0) - np.nanmean(respmat[idx_N2,:],axis=0),
        #                                        np.nanmean(respmat[idx_N3,:],axis=0))[0,1]
        corrdata[ialp,ises]      = np.corrcoef(np.nanmean(respmat[idx_N1,:],axis=0)/np.nanmean(respmat[idx_N2,:],axis=0),
                                               np.nanmean(respmat[idx_N3,:],axis=0))[0,1]
...
my_savefig(fig,figdir,'FF_FB_poprate_Corr_SP_%dsessions_boot' % nSessions)
...
# corrdata                = np.full((narealabelpairs,nSessions),np.nan)
# corrdata_boot                = np.full((narealabelpairs,nSessions,nboots),np.nan)
# fig,axes = plt.subplots(narealabelpairs,nSessions,figsize=(nSessions*3,narealabelpairs*3),sharey=True,sharex=True)
# for ises in range(nSessions):
#     respmat                = zscore(sessions[ises].calciumdata.T.to_numpy(), axis=1)
#     for ialp,alp in enumerate(arealabelpairs):
#         idx_N1              = np.all((sessions[ises].celldata['arealabel'] == alp.split('-')[0],
...
#         # corrdata[ialp,ises]      = np.corrcoef(np.nanmean(respmat[idx_N1,:],axis=0) - np.nanmean(respmat[idx_N2,:],axis=0),
#         #                                        np.nanmean(respmat[idx_N3,:],axis=0))[0,1]
#         xdata = np.nanmean(respmat[idx_N1,:],axis=0) -np.nanmean(respmat[idx_N2,:],axis=0)
#         ydata = np.nanmean(respmat[idx_N3,:],axis=0)
#         corrdata[ialp,ises]      = np.corrcoef(xdata,ydata)[0,1]
#         ax = axes[ialp,ises]
#         sns.regplot(x=xdata,y=ydata,ax=ax,color='k',
#                     scatter=True,marker='o',
#                     scatter_kws={'alpha':0.5, 's':20, 'edgecolors':'white'},
#                     line_kws={'color':'k', 'ls':'-', 'linewidth':3})
#         ax.set_title(alp)
